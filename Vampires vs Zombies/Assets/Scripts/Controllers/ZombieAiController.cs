...
public class ZombieAiController : BaseAiController {

    
    //private Character target;

    public ZombieAiController(Character c) : base(c) {
        trackingDist = 20f;
    ...
    public override void act() {
        ...
        if (target && (target.CharacterType == CharacterType.Zombie || target.Status == CharacterStatus.Paralysed || target.Status == CharacterStatus.Dead))
            target = null;

        if (Time.frameCount % 60 == 0 && (!target)) {
            //target = findTarget(c, CharacterType.Villager) ?? findTarget(c, CharacterType.Vampire);

            target = findTarget(c, CharacterType.Zombie); // prioritises villagers and enemies equally
        }
